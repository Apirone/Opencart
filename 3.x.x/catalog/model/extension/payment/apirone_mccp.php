...
define('SETTINGS_CODE', 'payment_apirone_mccp');
...
class ModelExtensionPaymentApironeMccp extends Model 
{
    ...
    public function getMethod(array $address): ?array
    {
        ...
        $_settings = $this->getSettings($this->update());
        if (!$_settings) {
            return null;
        }
     ...
     * @param mixed $log_level constants from LogLevel static class
     * @param string $message
     * @param ?array $context array of additional info if need
     */
    public function log(mixed $log_level, string $message, array $context = array()): void
    {
    ...
    /**
     * @param bool $updated plugin version has changed
     * @return Settings plugin settings
     */
    public function getSettings(bool $updated = false): ?Settings
    {
        if (self::$settings) {
    ...
    {
        return md5($this->getSettings()->secret . $salt);
    }
